...
public class MoveObjects : MonoBehaviour {
	...
	public static float height = 1.29f;
	...
	void Update () {
		...
		if (lastTriggerState && !currentTriggerState) {
			Vector3 forward = objectToMove[ObjectControl.ObjInx].forward;
			Vector3 up = Vector3.up;

			// If there is a valid raycast
			if (arcRaycaster!= null && arcRaycaster.MakingContact) {
				if (objectToMove[ObjectControl.ObjInx] != null) {
					if (teleportedUpAxis == UpDirection.TargetNormal) {
						up = arcRaycaster.Normal;
					}
					objectToMove[ObjectControl.ObjInx].position = arcRaycaster.HitPoint + up * height;
                   // DoneMove = true;
                }
			}
			...
			objectToMove[ObjectControl.ObjInx].rotation = Quaternion.LookRotation (forward, up);
